...
const ensureLegacyBonoImportColumns = async () => {
  ...
  if (!hasBonoTemplateId) {
    await prisma.$executeRawUnsafe(
      'ALTER TABLE "bono_packs" ADD COLUMN "bonoTemplateId" TEXT'
    )
  }
...
const readStoredBonoTemplatesForBackfill = async (): Promise<StoredBonoTemplateRow[]> => {
    ...
    return parsed
      .map((entry) => {
        ...
        return {
          ...
          serviceName: String((entry as any)?.serviceName || '').trim(),
          totalSessions: Number((entry as any)?.totalSessions || 0)
        }
      })
      .filter(
        (template) =>
...
const backfillBonoPackTemplateIds = async () => {
  if (!(await tableExists('bono_packs'))) {
    return
  }

  const templates = await readStoredBonoTemplatesForBackfill()
...
const ensureMultipleBonoSessionsPerAppointment = async () => {
...
export const ensureSqliteCompatibilityMigrations = async () => {
  ...
  await backfillBonoPackTemplateIds()
  ...
  await ensureMultipleBonoSessionsPerAppointment()
