...
interface ImportClientBonosModalProps {
  onSuccess: () => void
  onCancel: () => void
}
...
const instructions = [
  'Este importador carga bonos ya comprados por clientes, no el catálogo general de bonos.',
  'Se aceptan los formatos legacy .xls y .xlsx.',
  'Cada fila crea un bono en la ficha del cliente con sus sesiones restantes y consumidas.',
  'La importación no genera cobros ni movimientos de caja.'
]

export default function ImportClientBonosModal({
  onSuccess,
  onCancel
}: ImportClientBonosModalProps) {
  ...
  const postImport = async (mode: 'preview' | 'commit', createMissingClients?: boolean) => {
    ...
    try {
      ...
      const response = await api.post('/bonos/import-client-packs', formData, {
        headers: {
      ...
      if (mode === 'preview') {
        setPreview({
          ...
          depleted: normalizeSection(payload.depleted),
      ...
      const summaryParts = [
        nextResults.success.count > 0 ? `${nextResults.success.count} bonos importados` : null,
        ...
        nextResults.depleted.count > 0 ? `${nextResults.depleted.count} agotados` : null,
      ...
      console.error('Error importing client bonos:', error)
      toast.error(error.response?.data?.error || 'Error al importar bonos de clientes')
    } finally {
      setLoading(false)
    }
  }
  ...
  const previewDepleted = normalizeSection(preview?.depleted)
  ...
  const resultDepleted = normalizeSection(results?.depleted)
        ...
        <div className="flex items-start">
          ...
          <div>
            <h3 className="mb-2 text-sm font-semibold text-blue-900 dark:text-blue-200">
              Importación de bonos de clientes
            </h3>
      ...
      {preview && !results && (
        <div className="space-y-3">
          <div className="grid gap-3 md:grid-cols-2">
            ...
            {summaryCard('preview-depleted', 'Agotados', previewDepleted.count, 'blue', previewDepleted.items)}
            {summaryCard(
              'preview-missing',
          ...
          {hasMissingClients ? (
            <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 dark:border-amber-800 dark:bg-amber-900/20">
              <h4 className="text-sm font-semibold text-amber-900 dark:text-amber-200">
                Hay bonos con clientes sin ficha
              </h4>
              <p className="mt-1 text-sm text-amber-800 dark:text-amber-300">
                Puedes crear las fichas antes de importar o seguir omitiendo esos bonos.
              </p>
            ...
            <div className="rounded-lg border border-gray-200 bg-gray-50 p-3 text-sm text-gray-700 dark:border-gray-700 dark:bg-gray-700 dark:text-gray-200">
              Solo se importarán los bonos listos. Las filas ya importadas, agotadas o sin cliente se omiten.
            </div>
          )}
        </div>
      )}
      ...
      {results && (
        <div className="space-y-3">
          <div className="grid gap-3 md:grid-cols-2">
            ...
            {summaryCard('result-depleted', 'Agotados', resultDepleted.count, 'blue', resultDepleted.items)}
            {summaryCard(
              'result-missing',
              'Sin ficha de cliente',
          ...
          {importDecision !== null && (
            <div className="rounded-lg border border-gray-200 bg-gray-50 p-3 text-sm text-gray-700 dark:border-gray-700 dark:bg-gray-700 dark:text-gray-200">
              {importDecision
                ? 'Se han importado los bonos creando las fichas necesarias para los clientes ausentes.'
                : 'Se han omitido los bonos cuyos clientes seguían sin ficha.'}
            </div>
          )}
        </div>
      )}
