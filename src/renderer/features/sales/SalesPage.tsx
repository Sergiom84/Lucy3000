...
import {
  ...
  type CombinedSecondaryPaymentMethod,
...
export default function Sales() {
  ...
  const resolvedView = resolveSalesView(searchParams.get('view'))
  ...
  const pageBusy = loading || submitting

  const applyPendingCollectionSale = (sale: Sale) => {
    setPendingCollectionSale(sale)
    setPendingCollectionPaymentMethod('CASH')

    const remainingAmount = roundCurrency(Number(sale.pendingPayment?.amount || 0))
    setPendingCollectionAmount(remainingAmount > 0 ? remainingAmount.toFixed(2).replace('.', ',') : '')

    setSelectedClient(mapSaleClient(sale.client))
  }

  useEffect(() => {
    if (view === 'pos') {
      if (pendingSaleId) {
        void loadPendingCollectionSale(pendingSaleId).then((sale) => {
          if (!sale) {
            clearPendingCollectionQuery()
            return
          }

          applyPendingCollectionSale(sale)
        })
      } else {
    ...
    try {
      ...
      const saleStatus = combinedPayment
        ...
        : saleMode === 'PENDING'
          ? 'PENDING'
          : 'COMPLETED'
      const createdSale = await createSaleRecord({
  ...
  const executePendingCollection = async (options: PendingCollectionExecutionOptions) => {
    ...
    try {
      ...
      const updatedSale = await collectPendingSaleRecord(pendingCollectionSale.id, {
      ...
      setPendingCollectionExecution(null)
      setPendingCollectionCashDecisionModalOpen(false)
      applyPendingCollectionSale(updatedSale)
  ...
  const handleCompleteQuote = async () => {
    ...
    try {
      ...
      const createdQuote = await createQuoteRecord({
          ...
          <div className="space-y-4">
            ...
            <div className="card">
              ...
              <div className="space-y-4">
                ...
                <button onClick={handleCompleteSale} disabled={cart.length === 0 || pageBusy} className={`btn w-full ${saleMode === 'QUOTE' ? 'btn-secondary' : 'btn-primary'}`}>
                  {pageBusy
                    ? 'Procesando...'
                    : saleMode === 'QUOTE'
                      ? 'Generar Presupuesto'
          ...
          <div className="card">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="flex gap-2">
                ...
                />
                <button onClick={() => void loadSales(dateFilter)} className="btn btn-primary">
                  Buscar
                </button>
              </div>
            </div>
